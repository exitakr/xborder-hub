...
| 遊戯王 / 時計 / スニーカー / **高級車** | eBay Browse API | **出品価格** △ |
| 時計 / バッグ / スニーカー | **楽天市場**（無料・JPY 建て） | **出品価格** △ |
| バッグ（エルメス等） | 管理画面から手動登録（出典 URL 必須） | 買取価格表など ○ |
| 自動取得できない銘柄 | **利用者自身の評価額登録**（本人のみに反映） | 自己申告 △ |
| **全カテゴリ** | **利用者が投稿した売却実績** | **実売価格** ◎ |

Browse で一覧に無い銘柄は、利用者自身がその場でカタログに追加できます
（カテゴリに応じて自動で価格ソースを割り当て）。カタログの検索は日英
両方の表記（例: 「エルメス」でも「Hermès」でも）でヒットします。
