...
- [ ] `supabase/migrations/0020_brand_rules.sql`
      **ブランド別の日本語別名と下限価格**を自動適用します。
      利用者が追加した銘柄は `aliases` も `min_price` も空だったため、
      「エルメス」で検索できず、価格の異常値も素通りしていました。
      既存の全銘柄に遡って適用し、下限を下回っている価格は取り消します
      （次回の更新で再取得されます）
